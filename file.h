...
public:
    ...
    void allTickets(std::string status)
        {
   ...
   public:
       void updateOperatorInFile(const Ticket &ticket)
       {
           ...
           while (std::getline(inputFile, line))
           {
               ...
               { // Promjena operatera i statusa ako je pronadjen odgovarajuci tiket
                   data[1] = ticket.getStatus();
                   data[3] = ticket.getOperater();
               }

               for (int i = 0; i < data.size(); ++i)
               { // Dodavanje svakog tiketa (neizmjenjenih i izmijenjenog) u temp fajl
                   outputFile << data[i];
                   if (i < data.size() - 1)
                   {
                       outputFile << ",";
